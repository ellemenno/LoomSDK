...
class PackageTree
  ...
  def initialize(output_dir_root, api_output_dir)
    @output_dir_root = output_dir_root
    @api_output_dir = api_output_dir

    self.root_package = Module::PackageDoc.new("", @output_dir_root, @api_output_dir)
    root_package.path = ""
  end
  ...
  def []=( key, value )
    ...
    key.split(".").each_with_index do |super_package_path, index|
      if super_package.sub_packages[super_package_path.to_sym].nil?
        super_package.sub_packages[super_package_path.to_sym] = Module::PackageDoc.new(key.split(".")[0..index].join("."), @output_dir_root, @api_output_dir)
        super_package = super_package.sub_packages[super_package_path.to_sym]
      else
        super_package = super_package.sub_packages[super_package_path.to_sym]
      end
