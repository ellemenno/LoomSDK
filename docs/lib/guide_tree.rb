class GuideTree
  ...
  def initialize(output_dir_root, guides_output_dir)
    @output_dir_root = output_dir_root
    @guides_output_dir = guides_output_dir

    self.root = Module::TopicDoc.new("", @output_dir_root, @guides_output_dir)
  end
  ...
  def []=( key, value )
    ...
    key.split("/").each_with_index do |super_topic_path, index|
      if super_topic.sub_topics[super_topic_path].nil?
        super_topic.sub_topics[super_topic_path] = Module::TopicDoc.new(key.split("/")[0..index].join("/"), @output_dir_root, @guides_output_dir)
        super_topic = super_topic.sub_topics[super_topic_path]
      else
        super_topic = super_topic.sub_topics[super_topic_path]
      end
